    ...
    match op.flattened::<EntryTypes, LinkTypes>()? {
        FlatOp::StoreEntry(store_entry) => match store_entry {
            OpEntry::CreateEntry { app_entry, action } => match app_entry {
                EntryTypes::Houseseeker(houseseeker) => {
                    validate_create_houseseeker(EntryCreationAction::Create(action), houseseeker)
                }
            },
            OpEntry::UpdateEntry {
                app_entry, action, ..
                ...
                EntryTypes::Houseseeker(houseseeker) => {
                    validate_create_houseseeker(EntryCreationAction::Update(action), houseseeker)
                }
            },
            ...
            OpUpdate::Entry { app_entry, action } => {
                ...
                match app_entry {
                    EntryTypes::Houseseeker(houseseeker) => {
                        ...
                        let original_houseseeker = match Houseseeker::try_from(original_app_entry) {
                            Ok(entry) => entry,
                            Err(e) => {
                                return Ok(ValidateCallbackResult::Invalid(format!(
                                    "Expected to get Houseseeker from Record: {e:?}"
                                )));
                            }
                        };
                        validate_update_houseseeker(
                            action,
                            houseseeker,
                            original_create_action,
                            original_houseseeker,
                        )
                    }
                }
            }
        ...
        FlatOp::RegisterDelete(delete_entry) => {
            ...
            match original_app_entry {
                EntryTypes::Houseseeker(original_houseseeker) => validate_delete_houseseeker(
        ...
        FlatOp::StoreRecord(store_record) => {
            match store_record {
                ...
                OpRecord::CreateEntry { app_entry, action } => match app_entry {
                    EntryTypes::Houseseeker(houseseeker) => validate_create_houseseeker(
                ...
                OpRecord::UpdateEntry {
                    ...
                    match app_entry {
                        EntryTypes::Houseseeker(houseseeker) => {
                            let result = validate_create_houseseeker(
                                EntryCreationAction::Update(action.clone()),
                                houseseeker.clone(),
                            )?;
                            if let ValidateCallbackResult::Valid = result {
                                let original_houseseeker: Option<Houseseeker> = original_record
                                    .entry()
                                    .to_app_option()
                                    .map_err(|e| wasm_error!(e))?;
                                let original_houseseeker = match original_houseseeker {
                                    Some(houseseeker) => houseseeker,
                                    None => {
                                        return Ok(
                ...
                OpRecord::DeleteEntry {
                    ...
                    match original_app_entry {
                        EntryTypes::Houseseeker(original_houseseeker) => {
                            validate_delete_houseseeker(
                                action,
                                original_action,
                                original_houseseeker,
                            )
                ...
                OpRecord::CreateLink {
                    ...
                    LinkTypes::HouseseekerUpdates => validate_create_link_houseseeker_updates(
                        action,
                        base_address,
                        target_address,
                        tag,
