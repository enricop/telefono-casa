    ...
    match op.flattened::<EntryTypes, LinkTypes>()? {
        FlatOp::StoreEntry(store_entry) => match store_entry {
            OpEntry::CreateEntry { app_entry, action } => match app_entry {
                EntryTypes::UserProfile(user_profile) => {
                    validate_create_user_profile(EntryCreationAction::Create(action), user_profile)
                }
            },
            OpEntry::UpdateEntry {
                app_entry, action, ..
                ...
                EntryTypes::UserProfile(user_profile) => {
                    validate_create_user_profile(EntryCreationAction::Update(action), user_profile)
                }
            },
            ...
            OpUpdate::Entry { app_entry, action } => {
                ...
                match app_entry {
                    EntryTypes::UserProfile(user_profile) => {
                        ...
                        let original_user_profile = match UserProfile::try_from(original_app_entry)
                        {
                            Ok(entry) => entry,
                            Err(e) => {
                                return Ok(ValidateCallbackResult::Invalid(format!(
                                    "Expected to get UserProfile from Record: {e:?}"
                                )));
                            }
                        };
        ...
        FlatOp::RegisterDelete(delete_entry) => {
            ...
            match original_app_entry {
                EntryTypes::UserProfile(original_user_profile) => validate_delete_user_profile(
        ...
        FlatOp::RegisterCreateLink {
            ...
            LinkTypes::UserProfileUpdates => {
                validate_create_link_user_profile_updates(action, base_address, target_address, tag)
            }
        },
        FlatOp::RegisterDeleteLink {
            ...
            LinkTypes::UserProfileUpdates => validate_delete_link_user_profile_updates(
                action,
                original_action,
                base_address,
                target_address,
        ...
        FlatOp::StoreRecord(store_record) => {
            match store_record {
                ...
                OpRecord::CreateEntry { app_entry, action } => match app_entry {
                    EntryTypes::UserProfile(user_profile) => validate_create_user_profile(
                ...
                OpRecord::UpdateEntry {
                    ...
                    match app_entry {
                        EntryTypes::UserProfile(user_profile) => {
                            let result = validate_create_user_profile(
                                EntryCreationAction::Update(action.clone()),
                                user_profile.clone(),
                            )?;
                            if let ValidateCallbackResult::Valid = result {
                                let original_user_profile: Option<UserProfile> = original_record
                                    .entry()
                                    .to_app_option()
                                    .map_err(|e| wasm_error!(e))?;
                                let original_user_profile = match original_user_profile {
                ...
                OpRecord::DeleteEntry {
                    ...
                    match original_app_entry {
                        EntryTypes::UserProfile(original_user_profile) => {
                            validate_delete_user_profile(
                                action,
                                original_action,
                                original_user_profile,
                            )
                ...
                OpRecord::CreateLink {
                    ...
                    LinkTypes::UserProfileUpdates => validate_create_link_user_profile_updates(
                        action,
                        base_address,
                        target_address,
                        tag,
